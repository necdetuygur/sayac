const SOZ_LIST = `Sigara iradeyle değil mantıkla bırakılır.
...
Egzoz dumanları sigara içmekten daha çok zarar veriyor, peki azınızı bir egzoz borusuna dayayıp bilerek dumanları çeker misiniz?
...
Sigara içmeyen biri olarak kalmak için gereken tek şey bir daha sigara içmemektir.`.split(
  "\n"
);
...
function GetSoz() {
  var soz = null;
  while (!soz) {
    soz = SOZ_LIST[(Math.random() * SOZ_LIST.length).toFixed(0) * 1];
  }
  return soz;
}
